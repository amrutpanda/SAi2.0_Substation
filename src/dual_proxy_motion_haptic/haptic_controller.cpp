...
// clamp values
double clampValue(const double& val, const double& low, const double& high) {
	if (val < low) {
		return low;
	} else if (val > high) {
		return high;
...
const double max_vel = 0.1;  // velocity at which max damping is used
...
int main(int argc, char* argv[]) {
	...
	Vector3d robot_pos = Vector3d(0, 0, 0);
	...
	double kp_haptic = 0.15 * teleop_task->_max_linear_stiffness_device;
	double kv_haptic = 0.8 * teleop_task->_max_linear_damping_device;
	Vector3d haptic_center = Vector3d::Zero();

	Vector3d proxy_position_device_frame = Vector3d::Zero();

	Vector3d rotation_axis = Vector3d(0, 0, 0);
	int haptic_reset_center = 0;
	int robot_ready = 0;
	...
	redis_client_local.addIntToReadCallback(0, ROBOT_STATE_KEY, state);
	redis_client_local.addEigenToReadCallback(0, ROBOT_POS_KEY, robot_pos);
	redis_client_local.addEigenToReadCallback(0, ROBOT_ROT_KEY, robot_rot);
	redis_client_local.addIntToReadCallback(0, ROBOT_READY_STATE_KEY, robot_ready);
	// redis_client_local.addIntToReadCallback(0, HAPTIC_RESET_CENTER_KEY, haptic_reset_center);
	...
	// setup data loggingrotation_axis
	...
	while (runloop) {
		...
		if (state != prev_state) {
			...
			if (state == ORIENTATION) {
				haptic_center = teleop_task->_current_position_device;  // for rendering forces
			} else if (state == CLEANING) {
				haptic_center = teleop_task->_current_position_device;  // for rendering forces
			}
			if (prev_state == RECENTER) {
				// reset haptics center, robot center, and reference device base rotation frame 
		...
		if (state == INIT) {
			...
			if (teleop_task->device_homed && robot_ready && gripper_state) {
			...
			/*
				Task space control with force sensor feedback given by robot controller 
			*/
			...
			// compute damping based on haptic velocity 
			double device_vel = teleop_task->_current_trans_velocity_device.norm();
			double kv_ramp = teleop_task->_max_linear_damping_device * clampValue(device_vel / max_vel, 0, 1);

			// Vector3d desired_force = k_vir * delayed_sigma_force * (proxy_position_device_frame - device_position);  // proxy
			// Vector3d desired_force = k_vir * delayed_sigma_force * teleop_task->_Rotation_Matrix_DeviceToRobot * -(sensed_force_robot_frame - sensed_force_offset); 			
			Vector3d desired_force = 0.15 * teleop_task->_Rotation_Matrix_DeviceToRobot * -(sensed_force_robot_frame - sensed_force_offset) \
											- kv_ramp * teleop_task->_current_trans_velocity_device;
			...
			} else if (desired_force.norm() < min_force) {
				desired_force.setZero();
			}

			teleop_task->_commanded_force_device = desired_force;
			// teleop_task->_commanded_force_device = desired_force - teleop_task->_max_linear_damping_device * delayed_sigma_force * teleop_task->_current_trans_velocity_device;
			// teleop_task->_commanded_force_device = desired_force - 0.2 * teleop_task->_max_linear_damping_device * teleop_task->_current_trans_velocity_device;
			...
			/*
				Orientation control only; haptics input are still computed, but device is locked in position - no forces are rendered
			*/
			...
			Vector3d device_position = teleop_task->_current_position_device;
			proxy_position_device_frame = teleop_task->_home_position_device + teleop_task->_Rotation_Matrix_DeviceToRobot * (delayed_haptic_proxy - teleop_task->_center_position_robot) / Ks;
			// teleop_task->_commanded_force_device = - kp_haptic * (teleop_task->_current_position_device - haptic_center) - kv_haptic * delayed_sigma_force * teleop_task->_current_trans_velocity_device;
			teleop_task->_commanded_force_device = - kp_haptic * (teleop_task->_current_position_device - haptic_center) - kv_haptic * teleop_task->_current_trans_velocity_device;
			Vector3d desired_force = teleop_task->_commanded_force_device;
			...
			/*
				Cleaning state is actuated by external toggle switch; haptic inputs are still computed - no force feedback 
			*/

			// manual override of gripper state to zero 
			gripper_state = 0;
			gripper_state_prev = 0;
			...
			// teleop_task->_commanded_force_device = - kp_haptic * (teleop_task->_current_position_device - haptic_center) - kv_haptic * delayed_sigma_force * teleop_task->_current_trans_velocity_device;
			teleop_task->_commanded_force_device = - kp_haptic * (teleop_task->_current_position_device - haptic_center) - kv_haptic * teleop_task->_current_trans_velocity_device;
			// teleop_task->_commanded_force_device = desired_force - kv_haptic * delayed_sigma_force * teleop_task->_current_trans_velocity_device;  // position microadjustment 
			Vector3d desired_force = teleop_task->_commanded_force_device;
			...
			/*
				Haptics is recentered, but robot has no motion
			*/

			Vector3d desired_force = - 0.2 * kv_haptic * teleop_task->_current_trans_velocity_device;

			// saturate desired forces 
			Vector3d desired_force_diff = desired_force - prev_desired_force;
			if (desired_force_diff.norm() > max_force_diff) {
				desired_force = prev_desired_force + max_force_diff * desired_force_diff/desired_force_diff.norm();
			}
